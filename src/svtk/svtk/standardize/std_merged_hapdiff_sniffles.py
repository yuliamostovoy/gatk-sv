...
#
"""
std_merged_hapdiff_sniffles.py

Standardize merged hapdiff/sniffles records.
"""
import sys
...
@VCFStandardizer.register('merged_hapdiff_sniffles')
class MergeHSStandardizer(VCFStandardizer):
    """
    Skipping this for the merge standardization
    def standardize_records(self):
        "#""
        Filter merged hapdiff/sniffles VCF
    ...
    def standardize_info(self, std_rec, raw_rec):
        ...
        if std_rec.info['SVTYPE'] == 'INV':
            ...
            end = end + 1 # this is to fix an error in Sniffles ends being apparently 0-based
        ...
        std_rec.info['ALGORITHMS'] = ['merged_hapdiff_sniffles']
    ...
    def standardize_format(self, std_rec, raw_rec):
        ...
        Add GT if missing
        Add GQ
        If DR and DV, rename to RR and SR (num ref-supporting and variant-supporting reads)
